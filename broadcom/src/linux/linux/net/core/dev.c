...
#include <bcmnvram.h>			// for WL-330gE PPPoE connection under adapter/repeater mode
static int pppoe_relay_enable_set=0;
static int pppoe_relay_enable=0;
static u8 dev_cached_MAC[ETH_ALEN];
//static u8 client_cached_MAC[ETH_ALEN];
static u8 gw_cached_MAC[ETH_ALEN];
static int dev_cached_MAC_set=0;
static int gw_cached_MAC_set=0;
...
/* A PPPoE Packet, without Ethernet headers */

struct PPPoEPacketStruct_sp {
    unsigned int ver:4;		/* PPPoE Version (must be 1) */
    unsigned int type:4;	/* PPPoE Type (must be 1) */
    unsigned int code:8;	/* PPPoE code */
    unsigned int session:16;	/* PPPoE session */
...
static struct UNIQ_MAC_pair {
	unsigned char UNIQ[ETH_DATA_LEN];
	u8 MAC[ETH_ALEN];
	u8 used;
} uniq_mac_pair[16];

static struct SESSIONID_MAC_TABLE {
	u8 MAC[ETH_ALEN];
//} sessionid_mac_table[65535];
} sessionid_mac_table[256];	// 256 PPPoE sessions should be enough...
...
static struct PPPoEPacketStruct packet;
static unsigned char *curTag;
static unsigned char host_uniq[ETH_DATA_LEN];
static unsigned int len, tagType, tagLen;
static unsigned int len_host_uniq=0;

static struct PPPoEPacketStruct_sp packet_sp;
static unsigned char *curTag_sp;
static unsigned char host_uniq_sp[ETH_DATA_LEN];
static unsigned int len_sp, tagType_sp, tagLen_sp;
static unsigned int len_host_uniq_sp=0;
...
int dev_queue_xmit(struct sk_buff *skb)
{
	...
	if(!pppoe_relay_enable_set)
	{
		pppoe_relay_enable=(strcmp(nvram_safe_get("wl_mode_EX"), "ap") && strcmp(nvram_safe_get("wan_nat_X"), "1"))?1:0;
		pppoe_relay_enable_set=1;
	}
//	if(strcmp(nvram_safe_get("wl_mode_EX"), "ap") && strcmp(nvram_safe_get("wan_nat_X"), "1"))	// for WL-330gE PPPoE connection under adapter/repeater mode
	if(pppoe_relay_enable)
	{
		if (	!strcmp(skb->dev->name, "eth1") &&
			((skb->data[0] & skb->data[1] & skb->data[2] & skb->data[3] & skb->data[4] & skb->data[5]) == 0xFF || !memcmp(gw_cached_MAC, skb->data, ETH_ALEN)) &&
			((skb->data[6] & skb->data[7] & skb->data[8] & skb->data[9] & skb->data[10] & skb->data[11]) != 0xFF /*&& (!dev_cached_MAC_set || !memcmp(client_cached_MAC, skb->data+6, ETH_ALEN))*/)
		)
		{
/*
			printk("\nPPPoE message is relayed from WL-330gE %02x:%02x:%02x:%02x:%02x:%02x to AP.\n",
				skb->dev->dev_addr[0],
				skb->dev->dev_addr[1],
				skb->dev->dev_addr[2],
				skb->dev->dev_addr[3],
			...
			printk("\n");
			printk("data len:%d", skb->len);
			int dd;
			for(dd=0;dd<skb->len;dd++)
			{
				if(dd%16==0) printk("\n");
				printk("%02x ", (unsigned char)skb->data[dd]);
			};
			printk("\n\n");
*/			
			if (skb->mac.ethernet->h_proto == 0x6488)
				memcpy(skb->data+6, dev->dev_addr, ETH_ALEN);
			else if (skb->mac.ethernet->h_proto == 0x6388)
			{
				memcpy(&packet, skb->data, skb->len);
				len = ntohs(packet.length);
/*
				printk("PPPoE version: %d\n", (int) packet.ver);
				printk("PPPoE type: %d\n", (int) packet.type);
				printk("PPPoE length: %d\n", (int) ntohs(packet.length));
*/
				if (	packet.ver == 1 &&		/* PPPoE Version (must be 1) */
					packet.type == 1 &&		/* PPPoE Type (must be 1) */
					packet.code != 0 &&		/* We are not intersted in PPPOE Session packets */
					len <= ETH_DATA_LEN - 6		/* 6-byte overhead for PPPoE header */
				)	
				{	/* Step through the tags */
    					curTag = packet.payload;
    					while(curTag - packet.payload < len) {
					/* Alignment is not guaranteed, so do this by hand... */
						tagType = (((unsigned int) curTag[0]) << 8) + (unsigned int) curTag[1];
						tagLen = (((unsigned int) curTag[2]) << 8) + (unsigned int) curTag[3];
						if (tagType == TAG_END_OF_LIST) {
							break;
						}
						if ((curTag - packet.payload) + tagLen + TAG_HDR_SIZE > len) {
							break;	/* Invalid PPPoE tag length */
						}
						if (tagType == TAG_HOST_UNIQ) {
							memcpy(&len_host_uniq, curTag+2, 2);
							memset(&host_uniq, 0x0, ETH_DATA_LEN);
							memcpy(&host_uniq, curTag+TAG_HDR_SIZE, tagLen);
							break;
						}
				...
				if (ntohs(len_host_uniq)!=0)
				{
/*
					printk("PPPoE TAG_HOST_UNIQ length: %d\n", (int) ntohs(len_host_uniq));
					printk("PPPoE TAG_HOST_UNIQ data:");
					int ee;
					for(ee=0;ee<ntohs(len_host_uniq);ee++)
					{
						if(ee%16==0) printk("\n");
						printk("%02x ", (unsigned char)host_uniq[ee]);
					};
					printk("\n\n");
*/
					int cc;
					for(cc=0;cc<16;cc++)
					{
						if (memcmp(uniq_mac_pair[cc].UNIQ, host_uniq, ETH_DATA_LEN)==0)						
						{
							uniq_mac_pair[cc].used=1;
							last=cc;
							memcpy(uniq_mac_pair[cc].MAC, skb->data+6, ETH_ALEN);
							goto done;
						}
					}
					...
					for(cc=0;cc<16;cc++)
					{
						if (uniq_mac_pair[cc].used==0)					
						{
							uniq_mac_pair[cc].used=1;
							last=cc;
							memset(uniq_mac_pair[cc].UNIQ, 0x0, ETH_DATA_LEN);
							memcpy(uniq_mac_pair[cc].UNIQ, host_uniq, ETH_DATA_LEN);
							memcpy(uniq_mac_pair[cc].MAC, skb->data+6, ETH_ALEN);
							goto done;
						}
					}

					memset(uniq_mac_pair[(last+1)%16].UNIQ, 0x0, ETH_DATA_LEN);
					memcpy(uniq_mac_pair[(last+1)%16].UNIQ, host_uniq, ETH_DATA_LEN);
					memcpy(uniq_mac_pair[(last+1)%16].MAC, skb->data+6, ETH_ALEN);
					last=(last+1)%16;
done:
				}
				if (!dev_cached_MAC_set)
				{
					memcpy(dev_cached_MAC, skb->dev->dev_addr, ETH_ALEN);
//					memcpy(client_cached_MAC, skb->data+6, ETH_ALEN);
					dev_cached_MAC_set=1;
				}
...
//	if(strcmp(nvram_safe_get("wl_mode_EX"), "ap") && strcmp(nvram_safe_get("wan_nat_X"), "1"))	// for WL-330gE PPPoE connection under adapter/repeater mode
	...
	{
		if (	!strcmp(skb->dev->name, "eth1") &&
			!memcmp(dev_cached_MAC, skb->mac.ethernet->h_dest, ETH_ALEN)/* &&
			(skb->mac.ethernet->h_proto == 0x6388 || skb->mac.ethernet->h_proto == 0x6488)*/
		)
		{
/*
			int dd;
			printk("data(data) len:%d\n", skb->len);
			if (skb->len!=0)
			{
			for(cc=0;cc<skb->len;cc++)
			{
				if(cc%16==0) printk("\n");
...
*/
			if(skb->mac.ethernet->h_proto == 0x6488)
			{
				memcpy(&packet_sp, skb->data, skb->len);
				len_sp = ntohs(packet_sp.length);
/*
				printk("PPPoE version: %d\n", (int) packet_sp.ver);
				printk("PPPoE type: %d\n", (int) packet_sp.type);
				printk("PPPoE length: %d\n", (int) ntohs(packet_sp.length));
*/
				if (	packet_sp.ver == 1 &&		/* PPPoE Version (must be 1) */
					packet_sp.type == 1 &&		/* PPPoE Type (must be 1) */
					packet_sp.code == 0 &&		/* We are not intersted in PPPOE Session packets */
					len_sp <= ETH_DATA_LEN - 6		/* 6-byte overhead for PPPoE header */
				)	
				{
					memcpy(skb->mac.ethernet->h_dest, sessionid_mac_table[(ntohs(packet_sp.session))%256].MAC, ETH_ALEN);
/*
					printk("PPPoE Session ID: %d\n", (int) ntohs(packet_sp.session));
					printk("!!PPPoE message is relayed from WL-330gE to client %02x:%02x:%02x:%02x:%02x:%02x.\n",
...
//					goto target_done;
			...
			{
				if (!gw_cached_MAC_set)
				{
					memcpy(gw_cached_MAC, skb->mac.ethernet->h_source, ETH_ALEN);
					gw_cached_MAC_set=1;
				}
...
*/
				...
				{
					/* Step through the tags */
    					curTag_sp = packet_sp.payload;
	    				while(curTag_sp - packet_sp.payload < len_sp) {
					/* Alignment is not guaranteed, so do this by hand... */
						tagType_sp = (((unsigned int) curTag_sp[0]) << 8) + (unsigned int) curTag_sp[1];
						tagLen_sp = (((unsigned int) curTag_sp[2]) << 8) + (unsigned int) curTag_sp[3];
						if (tagType_sp == TAG_END_OF_LIST) {
							break;
						}
						if ((curTag_sp - packet_sp.payload) + tagLen_sp + TAG_HDR_SIZE > len_sp) {
							break;	/* Invalid PPPoE tag length */
						}
						if (tagType_sp == TAG_HOST_UNIQ) {
							memcpy(&len_host_uniq_sp, curTag_sp+2, 2);
							memset(&host_uniq_sp, 0x0, ETH_DATA_LEN);
							memcpy(&host_uniq_sp, curTag_sp+TAG_HDR_SIZE, tagLen_sp);
							break;
						}
						curTag_sp = curTag_sp + TAG_HDR_SIZE + tagLen_sp;
    					}
				}

				if (ntohs(len_host_uniq_sp)!=0)
				{
/*
					printk("PPPoE TAG_HOST_UNIQ length: %d\n", (int) ntohs(len_host_uniq_sp));
					printk("PPPoE TAG_HOST_UNIQ data:");
					int ee;
					for(ee=0;ee<ntohs(len_host_uniq_sp);ee++)
					{
						if(ee%16==0) printk("\n");
						printk("%02x ", (unsigned char)host_uniq_sp[ee]);
					};
					printk("\n\n");
*/
					int cc;
...
/*
						...
						{
							if(dd%16==0) printk("\n");
							printk("%02x ", (unsigned char)uniq_mac_pair[cc].UNIQ[dd]);
						}
						printk("\n");	
						printk("MAC: %02x:%02x:%02x:%02x:%02x:%02x.\n",
							uniq_mac_pair[cc].MAC[0],
							uniq_mac_pair[cc].MAC[1],
							uniq_mac_pair[cc].MAC[2],
							uniq_mac_pair[cc].MAC[3],
							uniq_mac_pair[cc].MAC[4],
							uniq_mac_pair[cc].MAC[5]
						);
*/
						if (uniq_mac_pair[cc].used==1 && memcmp(uniq_mac_pair[cc].UNIQ, host_uniq_sp, ETH_DATA_LEN/*ntohs(len_host_uniq_sp)*/)==0)
						{
							if (packet_sp.code==CODE_PADS)
							{
//								printk("!!!PPPoE Session ID: %d\n", (int) ntohs(packet_sp.session));
								
								memcpy(sessionid_mac_table[(ntohs(packet_sp.session))%256].MAC, uniq_mac_pair[cc].MAC, ETH_ALEN);
							}
							memcpy(skb->mac.ethernet->h_dest, uniq_mac_pair[cc].MAC, ETH_ALEN);
/*
							printk("!PPPoE message is relayed from WL-330gE to client %02x:%02x:%02x:%02x:%02x:%02x.\n",
								skb->mac.ethernet->h_dest[0],
								skb->mac.ethernet->h_dest[1],
...
/*
			memcpy(skb->mac.ethernet->h_dest, client_cached_MAC, ETH_ALEN);
			printk("PPPoE message is relayed from WL-330gE to client %02x:%02x:%02x:%02x:%02x:%02x.\n",
				client_cached_MAC[0],
				client_cached_MAC[1],
				client_cached_MAC[2],
