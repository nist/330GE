...
#define	DRIVER_VERSION "alpha (" "25 July 2003" ")"

static const char driver_name [] = "pxa2xx_udc";
static const char driver_desc [] = "PXA 2xx USB Device Controller driver";
...
#undef DEBUG	/* few strings, and little code to use them */
#endif
 ...
 * pxa250 a0/a1 b0/b1 sure act like they're still in c0 (pxa255 a0).
 */
...
if (*ep->reg_udccs & ~UDCCS_BI_TUR) printk ("W%02x\n", *ep->reg_udccs);
...
{
	/* implement and debug this when you need it */
	UDCCS0 = UDCCS0_FST;
	req->req.status = -ENOSYS;
	return 1;
}
...
pxa2xx_ep_queue(struct usb_ep *_ep, struct usb_request *_req, int gfp_flags)
{
	...
	/* 8KB per-request limit, from pxa dma controller */
	if (unlikely (req->req.length & ~DCMD_LENGTH))
		return -EMSGSIZE;
	...
	if (list_empty(&ep->queue) && !ep->stopped) {
		if (ep->desc == 0 /* ep0 */) {
			...
			case EP0_OUT_DATA_PHASE:
				...
				if (length == 0 && !dev->req_std)
					UDCCS0 = UDCCS0_IPR;
				if (dev->req_pending) {
					/* RNE "shouldn't" be set yet */
					ep0start(dev, 0, "OUT");
				}
		...
		int *eof, void *_dev)
{
	...
	t = snprintf(next, size, "ep0 IN %lu/%lu, OUT %lu/%lu; irqs %lu\n\n",
	...
	for (i = 0; i < 15; i++) {
		...
		if (i != 0) {
			...
			t = snprintf(next, size, "ep0 max 16 pio\n");
		if (t <= 0 || t > size)
			goto done;
		size -= t;
		next += t;
...
static void udc_reinit(struct pxa2xx_udc *dev)
{
	...
	dev->ep[ 0].reg_udccs      = &UDCCS0;
	dev->ep[ 0].reg_uddr       = &UDDR0;

	/* and three groups: */
	for (i = 1; i < 15; i += 5) {
		/* bulk in/out */
		dev->ep [i + 0].bEndpointAddress |= USB_DIR_IN;
		dev->ep [i + 0].ep.maxpacket = BULK_FIFO_SIZE;

		dev->ep [i + 1].ep.maxpacket = BULK_FIFO_SIZE;

		/* iso in/out */
		dev->ep [i + 2].bEndpointAddress |= USB_DIR_IN;
		dev->ep [i + 2].bmAttributes = USB_ENDPOINT_XFER_ISOC;
		dev->ep [i + 2].ep.maxpacket = ISO_FIFO_SIZE;

		dev->ep [i + 3].bmAttributes = USB_ENDPOINT_XFER_ISOC;
		dev->ep [i + 3].ep.maxpacket = ISO_FIFO_SIZE;

		/* interrupt in */
		dev->ep [i + 4].bEndpointAddress |= USB_DIR_IN;
		dev->ep [i + 4].bmAttributes = USB_ENDPOINT_XFER_INT;
		dev->ep [i + 4].ep.maxpacket = INT_FIFO_SIZE;
	}

	/* CHIPREV issue: before rev c0 (and pxa255) erratum 130 means
	 * dma is ONLY safe for transfers IN to the host.
	 *
	 * implementing OUT dma needs a bugfixed chip.
	 * and for bulk-out, dma chaining (UHCI-ish descriptor per packet).
	 * so to simplify, don't yet enable OUT dma even for iso-out.
	 */

	/* first */
	dev->ep[ 1].reg_udccs      = &UDCCS1;
	dev->ep[ 1].reg_uddr       = &UDDR1;
	dev->ep[ 1].reg_drcmr       = &DRCMR25;

	dev->ep[ 2].reg_udccs      = &UDCCS2;
	dev->ep[ 2].reg_ubcr       = &UBCR2;
	dev->ep[ 2].reg_uddr       = &UDDR2;
	// dev->ep[ 2].reg_drcmr       = &DRCMR26;

	dev->ep[ 3].reg_udccs      = &UDCCS3;
	dev->ep[ 3].reg_uddr       = &UDDR3;
	dev->ep[ 3].reg_drcmr       = &DRCMR27;

	dev->ep[ 4].reg_udccs      = &UDCCS4;
	dev->ep[ 4].reg_ubcr       = &UBCR4;
	dev->ep[ 4].reg_uddr       = &UDDR4;
	// dev->ep[ 4].reg_drcmr       = &DRCMR28;

	dev->ep[ 5].reg_udccs      = &UDCCS5;
	dev->ep[ 5].reg_uddr       = &UDDR5;

	/* second */
	dev->ep[ 6].reg_udccs      = &UDCCS6;
	dev->ep[ 6].reg_uddr       = &UDDR6;
	dev->ep[ 6].reg_drcmr       = &DRCMR30;

	dev->ep[ 7].reg_udccs      = &UDCCS7;
	dev->ep[ 7].reg_ubcr       = &UBCR7;
	dev->ep[ 7].reg_uddr       = &UDDR7;
	// dev->ep[ 7].reg_drcmr       = &DRCMR31;

	dev->ep[ 8].reg_udccs      = &UDCCS8;
	dev->ep[ 8].reg_uddr       = &UDDR8;
	dev->ep[ 8].reg_drcmr       = &DRCMR32;

	dev->ep[ 9].reg_udccs      = &UDCCS9;
	dev->ep[ 9].reg_ubcr       = &UBCR9;
	dev->ep[ 9].reg_uddr       = &UDDR9;
	// dev->ep[ 9].reg_drcmr       = &DRCMR33;

	dev->ep[10].reg_udccs      = &UDCCS10;
	dev->ep[10].reg_uddr       = &UDDR10;

	/* third */
	dev->ep[11].reg_udccs      = &UDCCS11;
	dev->ep[11].reg_uddr       = &UDDR11;
	dev->ep[11].reg_drcmr       = &DRCMR35;

	dev->ep[12].reg_udccs      = &UDCCS12;
	dev->ep[12].reg_ubcr       = &UBCR12;
	dev->ep[12].reg_uddr       = &UDDR12;
	// dev->ep[12].reg_drcmr       = &DRCMR36;

	dev->ep[13].reg_udccs      = &UDCCS13;
	dev->ep[13].reg_uddr       = &UDDR13;
	dev->ep[13].reg_drcmr       = &DRCMR37;

	dev->ep[14].reg_udccs      = &UDCCS14;
	dev->ep[14].reg_ubcr       = &UBCR14;
	dev->ep[14].reg_uddr       = &UDDR14;
	// dev->ep[14].reg_drcmr       = &DRCMR38;

	dev->ep[15].reg_udccs      = &UDCCS15;
	dev->ep[15].reg_uddr       = &UDDR15;
}
...
#define	enable_disconnect_irq()		enable_irq(LUBBOCK_USB_DISC_IRQ)
#define	disable_disconnect_irq()	disable_irq(LUBBOCK_USB_DISC_IRQ)
...
config_change:
			...
			if (u.r.bRequestType & USB_DIR_IN)
				...
				/* this 12.5.3 #8 rule prevents gadget
				 * drivers from ever STALLing control-OUT!!
				 * UDCCFR_ACM lets us at least NAK config
				 * changes until we're ready to ACK.
				 */
				if (!(dev->req_std && u.r.wLength == 0))
					UDCCS0 = UDCCS0_IPR;
			}
...
static void handle_ep(struct pxa2xx_ep *ep)
{
	...
	do {
		...
		udccs = *ep->reg_udccs;
		if (is_in) {	/* irq from TPC, SST, or (for ISO) TUR */
			tmp = UDCCS_BI_TUR;
...
#define CP15R0_PROD		0x000003f0
#define PXA25x			0x00000100
#define PXA210			0x00000120

#define CP15R0_REV_MASK		0x0000000f

#define CP15R0_PRODREV_MASK	0x000003ff

#define PXA255_A0		0x00000106	/* or PXA260_B1 */
#define PXA260_B0		0x00000105
...
static int __init init(void)
{
	...
	default:
		out_dma = 0;
		WARN("unrecognized product revision: %08x\n", chiprev);
	}
	...
	printk(KERN_INFO "%s, IRQ %d%s%s%s\n", driver_desc, IRQ_USB,
		dev->has_cfr ? "" : " (!cfr)",
		out_dma ? "" : " (!dma-out)",
		ep1315 ? "" : " (!ep13/15)"
		);
