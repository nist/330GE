...
static int getdevpathintvar(sb_t *sbh, const char *name);
...
static uint32 sb_pmu_alp_clock(sb_info_t *si);
static void sb_pmu_init_resources(sb_t *sbh);
...
/* gpiopullup/down init flag to prevent them from being init multiple times */
static bool sb_gpiopullinit = FALSE;
                       ...
                       uint bustype, void *sdh, char **vars, uint *varsz)
{
	...
	if (srom_var_init((sb_t *)si, si->sb.bustype, si->curmap, si->osh, vars, varsz)) {
		SB_ERROR(("sb_doattach: srom_var_init failed: bad srom\n"));
		return (NULL);
	}
#endif	/* SB_PCMCIA_SROM */

	/* Find out Crystal frequency and init PMU/PLL */
	if (si->sb.cccaps & CC_CAP_PMU) {

		/* Find out Crystal frequency and init PMU/PLL */
		if (!((si->sb.chip == BCM5354_CHIP_ID) && (devid == BCM4710_DEVICE_ID)))
			sb_pmuctl_pllinit((sb_t *)si, getintvar(NULL, "xtalfreq"));

		/* Initialize PMU resource up/down table */
		sb_pmu_init_resources((sb_t *)si);
	}
	...
	case PCI_BUS:
		...
		if ((si->sb.boardvendor = (uint16)getdevpathintvar(&si->sb, "boardvendor")) == 0)
			si->sb.boardvendor = w & 0xffff;
		else
			SB_ERROR(("Overriding boardvendor: 0x%x instead of 0x%x\n",
			          si->sb.boardvendor, w & 0xffff));
		if ((si->sb.boardtype = (uint16)getdevpathintvar(&si->sb, "boardtype")) == 0)
	...
	case JTAG_BUS:
		...
		if (!vars || ((si->sb.boardtype = getintvar(*vars, "prodid")) == 0))
	...
	if (!sb_gpiopullinit && si->sb.ccrev >= 20) {
		W_REG(osh, &cc->gpiopullup, 0);
		W_REG(osh, &cc->gpiopulldown, 0);
		sb_gpiopullinit = TRUE;
	}
...
sb_taclear(sb_t *sbh)
{
	...
	if (BUSTYPE(si->sb.bustype) == PCI_BUS) {
		...
		if (inband) {
			...
			if (si->sb.sonicsrev == SONICS_2_2)
				...
				imerrlog = sb_corereg(si, si->sb.buscoreidx, SBIMERRLOG, 0, 0);
				if (imerrlog & SBTMEL_EC) {
					imerrloga = sb_corereg(si, si->sb.buscoreidx, SBIMERRLOGA,
						0, 0);
					/* clear errlog */
					sb_corereg(si, si->sb.buscoreidx, SBIMERRLOG, ~0, 0);
	...
	} else if (BUSTYPE(si->sb.bustype) == SDIO_BUS) {
...
BCMINITFN(sb_pci_setup)(sb_t *sbh, uint coremask)
{
	...
	if (PCIE(si)) {
		if ((si->sb.buscorerev == 0) || (si->sb.buscorerev == 1)) {
			reg_val = sb_pcie_readreg((void *)(uintptr)sbh,
		...
		if (si->sb.buscorerev == 1) {
			reg_val = sb_pcie_readreg((void *)(uintptr)sbh,
			                          ...
			                          PCIE_DLLP_LCREG);
			reg_val |= (0x40);
			sb_pcie_writereg(sbh, (void *)(uintptr)PCIE_PCIEREGS, PCIE_DLLP_LCREG,
			                 reg_val);
		}
...
BCMINITFN(sb_alp_clock)(sb_t *sbh)
{
	...
	cap = sb_corereg(si, SB_CC_IDX, OFFSETOF(chipcregs_t, capabilities), 0, 0);

	if (cap & CC_CAP_PMU)
		return sb_pmu_alp_clock(si);
	else
		return ALP_CLOCK;
}
...
sb_clkctl_fast_pwrup_delay(sb_t *sbh)
{
	...
	if ((R_REG(si->osh, &cc->capabilities) & CC_CAP_PMU)) {
		...
		/* Delay depends on lowest state PMU will drop to */
		minresmask = R_REG(si->osh, &cc->min_res_mask);
		if (minresmask & PMU4328_HT)
			fpdelay = 0;
		else if (minresmask & PMU4328_ALP)
			fpdelay = PMU_ALP_PLL_DLY;
		else
			fpdelay = PMU_ILP_PLL_DLY;
...
sb_d11_devid(sb_t *sbh)
{
	...
	if (chip == BCM4328_CHIP_ID &&
	    (chippkg == BCM4328USBDUAL_PKG_ID || chippkg == BCM4328SDIODUAL_PKG_ID))
		device = BCM4328_D11DUAL_ID;
	else
#endif	/* BCM4328 */
	/* Let nvram variable override core ID */
	if ((device = (uint16)getdevpathintvar(sbh, "devid")) != 0)
		;
	/*
	 * no longer support wl0id, but keep the code
	 * here for backward compatibility.
	 */
	else if ((device = (uint16)getintvar(NULL, "wl0id")) != 0)
		;
	/* Chip specific conversion */
	else if (chip == BCM4712_CHIP_ID) {
...
void
BCMINITFN(sb_set_switcher_voltage)(sb_t *sbh, uint8 bb_voltage, uint8 rf_voltage)
{
	chipcregs_t *cc;
	uint origidx;
	osl_t *osh;
...
static const pmu_res_updown_t BCMINITDATA(pmu4328r0_res_updown)[] = {
	{ 0, 0x0101 },
	{ 1, 0x1f01 },
	{ 2, 0x010f },
	{ 3, 0x0101 },
...
static void
BCMINITFN(sb_pmu_init_resources)(sb_t *sbh)
{
	sb_info_t *si;
	chipcregs_t *cc;
	uint origidx;
	...
	/* PMU (clock switching) functionality is not used in 5354 */
	if (sb_chip(sbh) == BCM5354_CHIP_ID)
		return;
	...
	case BCM4328_CHIP_ID:
		...
		pmu_res_updown_table = pmu4328r0_res_updown;
		pmu_res_updown_table_sz = ARRAYSIZE(pmu4328r0_res_updown);
		break;
#endif	/* BCM4328 */
	...
	if (!pmu_res_updown_table)
		SB_MSG(("No PMU resources up/down time table found"
			"for chip %x rev %d, using hardware defaults\n",
			si->sb.chip, si->sb.chiprev));
	else {
		ASSERT(pmu_res_updown_table_sz);
	...
	SB_MSG(("PMU min resource mask: 0x%x\n", min_mask));
	/* program max resource mask */
	W_REG(osh, &cc->max_res_mask, max_mask);
	SB_MSG(("PMU max resource mask: 0x%x\n", max_mask));
...
uint16
BCMINITFN(sb_pmu_fast_pwrup_delay)(sb_t *sbh)
{
	sb_info_t *si;
	uint16 delay = 10000;
	...
	default:
		SB_MSG(("No PMU fast power up delay specified"
			"for chip %x rev %d, using default %d us\n",
			si->sb.chip, si->sb.chiprev, delay));
		break;
	}
...
static const xtaltab_t BCMINITDATA(xtaltab)[] = {
	{ 12000,	1,	73,	349525 },
	{ 13000,	2,	67,	725937 },
	{ 14400,	3,	61,	116508 },
	{ 15360,	4,	57,	305834 },
...
#ifdef BCMUSBDEV
#define	XTAL_DEFAULT	11
#else
#define XTAL_DEFAULT	8
#endif
 ...
 */
void
BCMINITFN(sb_pmuctl_pllinit)(sb_t *sbh, uint32 xtal)
{
	uint32 tmp, oldmin, oldmax;
	chipcregs_t *cc;
	const xtaltab_t *xt;
	uint origidx;
	osl_t *osh;
	...
	for (xt = xtaltab; xt->freq; xt++)
		if (xt->freq == xtal)
			break;
	if (xt->freq == 0)
		xt = &xtaltab[XTAL_DEFAULT];
	...
	SB_MSG(("IN PLL, XTAL %d (%d), origidx %d, cc %p\n", xtal, xt->xf, origidx, cc));
	osh = sb_osh(sbh);

	/* Check current PLL state */
	tmp = (R_REG(osh, &cc->pmucontrol) & PCTL_XTALFREQ_MASK) >> PCTL_XTALFREQ_SHIFT;
	if (tmp == xt->xf) {
		SB_MSG(("PLL already programmed for %d.%d MHz\n",
			(xt->freq / 1000), (xt->freq % 1000)));
	} else {
		ASSERT(0 == tmp);
		SB_MSG(("Programming PLL for %d.%d MHz\n", (xt->freq / 1000), (xt->freq % 1000)));

		/* Make sure the PLL is off */
		oldmin = R_REG(osh, &cc->min_res_mask);
		oldmax = R_REG(osh, &cc->max_res_mask);
		W_REG(osh, &cc->min_res_mask, oldmin & ~PMURES_BIT(PMU4328_BB_PLL_PU));
		W_REG(osh, &cc->max_res_mask, oldmax & ~PMURES_BIT(PMU4328_BB_PLL_PU));
		SPINWAIT(R_REG(osh, &cc->clk_ctl_st) & CCS0_HTAVAIL, 2 * PMU_ALP_PLL_DLY);
		ASSERT(!(R_REG(osh, &cc->clk_ctl_st) & CCS0_HTAVAIL));

		SB_MSG(("Done masking\n"));

		/* Write PDIV in pllcontrol[0] */
		W_REG(osh, &cc->pllcontrol_addr, PLLCTL0);
		tmp = R_REG(osh, &cc->pllcontrol_data);
		if (xt->freq >= PDIV_FREQ)
			tmp |= PC0_PDIV_MASK;
		else
			tmp &= ~PC0_PDIV_MASK;
		...
		W_REG(osh, &cc->pllcontrol_addr, PLLCTL1);
		tmp = R_REG(osh, &cc->pllcontrol_data);
		tmp = ((tmp & ~(PC1_WILD_INT_MASK | PC1_WILD_FRAC_MASK)) | \
			(((xt->wbint << PC1_WILD_INT_SHIFT) & PC1_WILD_INT_MASK) | \
				((xt->wbfrac << PC1_WILD_FRAC_SHIFT) & PC1_WILD_FRAC_MASK)));
		if (xt->wbfrac == 0)
			tmp |= PC1_STOP_MOD;
		else
			tmp &= ~PC1_STOP_MOD;
		...
		W_REG(osh, &cc->pllcontrol_addr, PLLCTL2);
		tmp = R_REG(osh, &cc->pllcontrol_data);
		tmp = ((tmp & ~PC2_WILD_INT_MASK) |
			((xt->wbint >> PC2_WILD_INT_SHIFT) & PC2_WILD_INT_MASK));
		W_REG(osh, &cc->pllcontrol_data, tmp);
		...
		/* Now turn PLL on */
		if (R_REG(osh, &cc->pmucontrol) & PCTL_ILP_DIV_EN)
			W_REG(osh, &cc->max_res_mask, 0xfffff);
		else
			W_REG(osh, &cc->max_res_mask, oldmax | PMURES_BIT(PMU4328_BB_PLL_PU));
		W_REG(osh, &cc->min_res_mask, oldmin | PMURES_BIT(PMU4328_BB_PLL_PU));
	}
...
uint32
sb_pmuctl_force_ilp(sb_t *sbh, bool force)
{
	chipcregs_t *cc;
	uint origidx;
	...
	if (force)
		W_REG(osh, &cc->pmucontrol, oldpmucontrol &
			~(uint32)(PCTL_HT_REQ_EN | PCTL_ALP_REQ_EN));
	else
		W_REG(osh, &cc->pmucontrol, oldpmucontrol |
...
void
sb_pmu_set_res_mask(sb_t *sbh, uint32 *min_mask, uint32 *max_mask)
{
	chipcregs_t *cc;
	uint origidx;
	uint32 oldmask;
	...
	/* Set new mask values */
	if (min_mask) {
		oldmask = R_REG(osh, &cc->min_res_mask);
		W_REG(osh, &cc->min_res_mask, *min_mask);
		*min_mask = oldmask;
	}
	if (max_mask) {
		oldmask = R_REG(osh, &cc->max_res_mask);
		W_REG(osh, &cc->max_res_mask, *max_mask);
		*max_mask = oldmask;
	}
...
static uint32
BCMINITFN(sb_pmu_alp_clock)(sb_info_t *si)
{
	uint32 xf;
	const xtaltab_t *xt;

	/* Get XtalFreq */
	xf = (sb_corereg(si, SB_CC_IDX, OFFSETOF(chipcregs_t, pmucontrol), 0, 0) &
	      PCTL_XTALFREQ_MASK) >> PCTL_XTALFREQ_SHIFT;
