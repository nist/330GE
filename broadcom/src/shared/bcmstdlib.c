 ...
 * $Id: bcmstdlib.c,v 1.1.1.1 2007/01/25 12:52:12 jiahao_jhou Exp $
 */
...
#include <stdarg.h>
	...
	defined(PCBIOS) || defined(LINUXSIM)
/* debatable */
#include <osl.h>
#else
#include <stdio.h>
...
unsigned long
BCMROMFN(strtoul)(char *cp, char **endp, int base)
{
	ulong result, value;
	bool minus;
...
printf(const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	count = vsnprintf(buffer, 512, fmt, ap);
	va_end(ap);
