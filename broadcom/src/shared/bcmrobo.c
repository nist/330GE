 ...
 *
 * $Id: bcmrobo.c,v 1.1.1.1 2007/01/25 12:52:12 jiahao_jhou Exp $
 */
...
mii_rreg(robo_info_t *robo, uint8 page, uint8 reg, void *val, int len)
{
	...
	case 4:
		*(uint32 *)val = val16;
		hi16 = robo->miird(h, PSEUDO_PHYAD, REG_MII_DATA1);
		*(uint32 *)val += hi16 << 16;
		break;
	}
...
#endif	/* _CFE_ */
		miiwr(h, PSEUDO_PHYAD, REG_MII_PAGE, 0xa501);
		tmp = miird(h, PSEUDO_PHYAD, REG_MII_PAGE);
		ET_MSG(("%s: read after write (0xa501) of mii page register: 0x%x\n",
		        __FUNCTION__, tmp));
		rc = mii_rreg(robo, PAGE_MMR, REG_DEVICE_ID, &robo->devid, 2);
...
bcm_robo_enable_device(robo_info_t *robo)
{
	...
	if ((reset = getgpiopin(robo->vars, "robo_reset", GPIO_PIN_NOTDEFINED)) !=
		...
		bcm_mdelay(20);
	} else if (sb_setcore(robo->sbh, SB_ROBO, 0)) {
		/* If we have an internal robo core, reset it using sb_core_reset */
		ET_MSG(("%s: Resetting internal robo core\n", __FUNCTION__));
		sb_core_reset(robo->sbh, 0, 0);
		sb_setcoreidx(robo->sbh, idx);
	} else if ((robo->devid == DEVID5397) || (robo->devid == DEVID5398)) {
...
/* Configure the VLANs */
int
bcm_robo_config_vlan(robo_info_t *robo)
{
	uint8 val8;
	uint16 val16;
	uint32 val32;
...
//	val8 = ((1 << 2) |		/* enable RSV multicast V Fwdmap */
	val8 = ((0 << 2) |		/* enable RSV multicast V Fwdmap */	/* enable flow control */
//		(1 << 3));		/* enable RSV multicast V Untagmap */
		(0 << 3));		/* enable RSV multicast V Untagmap */	/* enable flow control */
	if (robo->devid == DEVID5325)
...
vlan_setup:
		...
		if (robo->devid == DEVID5325) {
			...
			robo->ops->write_reg(robo, PAGE_VTBL, REG_VTBL_ENTRY, &val32,
			                     sizeof(val32));
