...
__devinit bcm5700_init_board(struct pci_dev *pdev, struct net_device **dev_out, int board_idx)
{
	...
	*dev_out = NULL;
	...
	if (pDevice->Flags & ROBO_SWITCH_FLAG) {
		...
		if ((pUmDevice->boardflags & BFL_ENETVLAN) && bcm_robo_config_vlan(robo)) {
...
robo_fail:
	...
	if (sbh) {
		...
		sprintf(etXmacaddr, "et%umacaddr", sbgige);
...
STATIC int bcm5700_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	case SIOCDEVPRIVATE + 3:		/* Read the specified ROBO register. */
	{
		...
		if (robo->ops->read_reg(robo, data[0] & 0xff, data[1] & 0xff, &value, 2))
			return -EIO;
	...
	case SIOCDEVPRIVATE + 4:		/* Write the specified ROBO register. */
	{
		robo_info_t *robo = (robo_info_t *)pUmDevice->robo;
		...
		value = data[2];
		if (robo->ops->write_reg(robo, data[0] & 0xff, data[1] & 0xff, &value, 2))
			return -EIO;
