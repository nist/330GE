...
chipattach(etc_info_t *etc, void *osh, void *regsva)
{
	...
	if (((boardflags & BFL_ENETROBO) ||
		...
		if ((boardflags & BFL_ENETVLAN) && bcm_robo_config_vlan(etc->robo)) {
...
chipinit(struct bcm4xxx *ch, uint options)
{
	...
	/* EPHY trimming, specific to 5354 chip */
	if (etc->chip == BCM5354_CHIP_ID) {
#define	PTEST		0x17	/* Phy register */
#define	PTEST_VAL	0x11	/* Default value */
		char	*etrimm;
		uint16	val;

		/*
		 * For each phy, write to EPHY PTEST register (addr:0x17)
		 * for EPHY trimming, value to be written comes from nvram,
		 * with 0x11 (PTEST_VAL above, DVT provided) as default.
		 */

		if ((etrimm = getvar(ch->vars, "etrimm")) != NULL)
			val = bcm_strtoul(etrimm, NULL, 0);
		else
			val = PTEST_VAL;

		chipphywr(ch, 0, PTEST, val);
		chipphywr(ch, 1, PTEST, val);
		chipphywr(ch, 2, PTEST, val);
		chipphywr(ch, 3, PTEST, val);
		chipphywr(ch, 4, PTEST, val);
	}
...
chipwrcam(struct bcm4xxx *ch, struct ether_addr *ea, uint camindex)
{
	...
	if ((w = ((R_REG(ch->osh, &ch->regs->camcontrol) & (CC_CB | CC_CE)))) != 0) {
		ET_ERROR(("chipwrcam: camcontrol is 0x%x\n", w));
	}

	w = (ea->octet[2] << 24) | (ea->octet[3] << 16) | (ea->octet[4] << 8) | ea->octet[5];
