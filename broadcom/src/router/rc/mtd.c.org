 ...
 *
 * $Id: mtd.c,v 1.1.1.1 2007/01/25 12:52:21 jiahao_jhou Exp $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
mtd_write(const char *path, const char *mtd)
{
	...
	for (erase_info.start = 0; erase_info.start < trx.len; erase_info.start += count) {
		...
		(void) ioctl(mtd_fd, MEMUNLOCK, &erase_info);
                ...
                fprintf(stderr, "Debug::mtd_fd=%d , buf=%s , count=%d\n", mtd_fd, buf, count);
                printf("Debug::mtd_fd=%d , buf=%s , count=%d\n", mtd_fd, buf, count);


		if (
#ifndef CONFIG_WL331G
