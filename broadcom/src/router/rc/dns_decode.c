 ...
 * This function assumes that buf points to an encoded name.
 * The name is decoded into name. Name should be at least 255 bytes long.
 * Add by Chen-I to process name with pointer
 */
void dns_decode_name_all(char *name, char **buf, int size, char *header)
{
  int i, k, len, j, offset;
  char *start;
  
  //printf("Decode Name: %x %x\n", size, *header);
  
  i = k = 0;  
  start = *buf;
  
  while( (*buf - start)<size )
  {
  	//printf("Off: %x %x\n", (unsigned char)**buf, *buf-start);
  	if ((**buf&0xc0)==0x00)	// Plain text
  	{
        	len = *(*buf)++;
        	for( j = 0; j<len ; j++)
	     	   name[k++] = *(*buf)++;  
...
void dns_decode_request(dns_request_t *m)
{
  ...
  for( i = 0; i < m->message.header.ancount && i < NUM_RRS; i++){
	...
	break;
      ...
      else if( m->message.answer[i].type == MX)
      {
      	int pref;
      	
	/* Reverse lookup so convert data to a nume */
	/* Get the first name, so you don't need to resolve recursively */
	ptr = m->message.answer[i].data;
	SET_UINT16(pref, &ptr);
	dns_decode_name_all( m->cname, &ptr, m->message.answer[i].rdatalen-2, m->original_buf);
	//printf("Name: %s\n", m->cname);
	break;
      }
