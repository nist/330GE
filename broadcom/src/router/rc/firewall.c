 ...
 *
 * $Id: firewall.c,v 1.1.1.1 2007/01/25 12:52:21 jiahao_jhou Exp $
 */
...
start_firewall(void)
{
	...
	if(nvram_match("wl_guest_ENABLE", "1") && nvram_match("wl_mode_EX", "ap") && nvram_match("wan_nat_X", "1"))
	{
		...
		if (nvram_match("mssid_ENABLE", "1"))
		{
		// accept dhcp packet only.
		memset(&filter, 0, sizeof(filter));
		filter.match.state = NETCONF_NEW;
		strcpy(filter.match.in.name, "wl0.2");
		filter.target = log_accept;
		add_filter(&filter, NETCONF_IN);
		add_filter(&filter, NETCONF_FORWARD);
		...
		memset(&filter, 0, sizeof(filter));
		filter.match.state = NETCONF_NEW;
		strcpy(filter.match.in.name, "wl0.3");
		filter.target = log_accept;
		add_filter(&filter, NETCONF_IN);
		add_filter(&filter, NETCONF_FORWARD);
		}
