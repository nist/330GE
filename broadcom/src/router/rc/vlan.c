 ...
 */
/*
 * IEEE 802.1Q Virtual LAN services
 *
 ...
 *
 * $Id: vlan.c,v 1.1.1.1 2007/01/25 12:52:21 jiahao_jhou Exp $
 */
#ifdef WL500GX
#include <stdio.h>
#include <stdlib.h>
...

{
    ...
    char *wan_hwaddr = nvram_safe_get("wan0_hwaddr"); //Thanks for Oleg
    ...
    if (!bcm_is_robo())
      return -1;
    bVlan = (atoi(vlan_enable) > 0) ||(strlen(vlan_enable) &&
    ...
    /* note that if brcm tags are enabled, the WAN is configured with a vlan and */
    /* the LAN ports are configured with the brcm tag driver.  if brcm tags are disabled */
    /* the LAN is configured with a vlan and the WAN directly uses eth0.  This */
    /* only applies to vlan interfaces created with vconfig.  In both cases, the underlying */
    /* switch is programmed with a separate vlan for WAN and LAN, but in the case w/o */
    /* brcm tags, the WAN vlan tag is removed when it exits the switch to the CPU */

    /* modify lan_ifbase & wan_ifbase for brcm tag driver, but save to bring up interface */
    ...
    if (bBrcmTag)
	{
        ...
        /* need to create wan vlan name here.  the form of the name will be ethx.port.vlan */
	...
	/* we need to handle the case of if bridge exists or not.  if
	 * it does exist, and brcm tags are enabled brcm tag (for default) interface names will be
     * substituted for 'ethx' name.  if it doesn't exist, bridge 'br0' will be created
	 * with only vlan or brcm tag interface names
	 */
	if (hasBridge) {
	...
	for (i = vlan_boards[board_index].lan_port_start;
        ...
        if (!bBrcmTag)
            /* if no brcm tag driver, just use one interface for all ports on LAN side */
            break;
	}
    ...
    /* change wan_hwaddr to be lan_hwaddr.  this is because underlying
     * lan device has lan_hwaddr and if wan vlan device had different
     * hwaddr, the vlan processing would set the underlying device to
     * promiscuous mode
     */
    ...
    if (bBrcmTag)
        ...
        /* don't configure WAN as vlan interface, just program switch with vlan id */
        bcm_vlan_create(0,wan_vid); /* this is for default case, which doesn't call vconfig */
    ...
    /* MII port is set up to tag with wan vlan here, but that will be overridden below */
    /* to tag with lan vlan (only matters for default case where CPU doesn't have vlan */
    /* configured, but switch does; ie, mii port does tag/untag) */
    sprintf(port,"%d",BCM_MII_PORT); /* mii port, note that vlan tags retained on egress */
    if (bBrcmTag)
	...
	if (bBrcmTag)
		/* if default mode, allow tagging of frames into MII.  this is to allow */
		/* lan to have vlan handling in switch, but remove vlan tag when received */
		/* by CPU */
		buf[0] |= 0x80;
    ...
    if (bBrcmTag)
    {
        ...
        bcm_write_reg(0,ROBO_VLAN_PAGE,ROBO_VLAN_CTRL1,buf,1);
        buf[0] = 0x8e;      /* MII port as management port, IGMP snoop, rx BPDUs */
    ...
    for (i = vlan_boards[board_index].lan_port_start;
        ...
        if (bBrcmTag) {
			/* default mode (w/brcm tags), create brcm tag device per port and set up */
            /* single vlan for all ports */
			if (i == vlan_boards[board_index].lan_port_start)
			{
                bcm_vlan_create(0,lan_vid); /* this is for default case, which doesn't call vconfig */
    ...
    if (!bBrcmTag)
    {
        ...
        /* set bogus mc addr to bypass specian h/w handling of BPDUs and let ARL handle them */
