 ...
 *
 * $Id: netconf.h,v 1.1.1.1 2007/01/25 12:53:02 jiahao_jhou Exp $
 */
...
#ifdef CONFIG_NETMAP
	NETCONF_NETMAP,			/* NETMAP for starcraft */
#endif
	NETCONF_TARGET_MAX
};
...
/* NETMAP port forward description */
typedef struct _netconf_app_t {
	NETCONF_FW_COMMON;
	uint16 proto;			/* Related protocol */
	uint16 sport[2];		/* Related destination port(s) (network order) */
	uint16 to[2];			/* Port(s) to map related destination port to (network order) */
} netconf_netmap_t;
#endif
