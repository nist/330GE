...
// definition of  	BYTE Regulation in struct PktGetInfo
enum  REG_DOMAIN
{
	REG_0x10US = 0x10,
	REG_0x30DE = 0x30,
...
#endif
		     strcpy(ginfo->PrinterInfo+1, nvram_safe_get("hardware_version"));	// add for WL-330gE ATE function
		     strcpy(ginfo->SSID, ssid_g);
		     if(nvram_match("wan_nat_X", "0") && nvram_match("lan_proto_X", "1") && nvram_invmatch("lan_netmask_t", ""))
			strcpy(ginfo->NetMask, nvram_safe_get("lan_netmask_t"));
		     else
...
//		     ginfo->Regulation = 0xff;
			if ((nvram_match("regulation_domain","0x47TW"))||(nvram_match("regulation_domain","0X47TW")))	// add for WL-330gE ATE function
				ginfo->Regulation = REG_0x47TW;
			else if ((nvram_match("regulation_domain","0x11HK"))||(nvram_match("regulation_domain","0X11HK")))
				ginfo->Regulation = REG_0x11HK;
			else if ((nvram_match("regulation_domain","0x45KR"))||(nvram_match("regulation_domain","0X45KR")))
				ginfo->Regulation = REG_0x45KR;
			else if ((nvram_match("regulation_domain","0x44CN"))||(nvram_match("regulation_domain","0X44CN")))
				ginfo->Regulation = REG_0x44CN;
			else if ((nvram_match("regulation_domain","0x10US"))||(nvram_match("regulation_domain","0X10US")))
				ginfo->Regulation = REG_0x10US;
			else if ((nvram_match("regulation_domain","0x30DE"))||(nvram_match("regulation_domain","0X30DE")))
				ginfo->Regulation = REG_0x30DE;
			else if ((nvram_match("regulation_domain","0x40JP"))||(nvram_match("regulation_domain","0X40JP")))
				ginfo->Regulation = REG_0x40JP;
			else if ((nvram_match("regulation_domain","0x21AU"))||(nvram_match("regulation_domain","0X21AU")))
				ginfo->Regulation = REG_0x21AU;
			else if ((nvram_match("regulation_domain","0x32FR"))||(nvram_match("regulation_domain","0X32FR")))
				ginfo->Regulation = REG_0x32FR;
			else
				ginfo->Regulation = 0xff;
